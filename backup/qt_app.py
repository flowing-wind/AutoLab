...
if not DEBUG_MODE:
    ...
    try:
        ...
        print(f"仪器连接错误: {e}")
        # DEBUG_MODE = True  # 如果连接失败，强制使用仿真模式
...
def get_setpoint():
    if DEBUG_MODE:
        # 仿真模式
        return sim_system.get_setpoint()
    else:
        # 实际仪器读取
        # 没写
        return 0
...
def get_stable_flag():
    if DEBUG_MODE:
        # 仿真模式
        return sim_system.get_stable_flag()
    else:
        # 实际仪器相关功能
        return 0

# ==================== 设置更新标志函数 ====================
def set_update_flag(flag_value):
    if DEBUG_MODE:
        # 仿真模式
        sim_system.set_update_flag(flag_value)
    else:
        # 实际仪器相关功能
        pass
...
class TemperatureMonitor(QMainWindow):
    def __init__(self):
        ...
        # 数据存储
        self.MAX_DATA_POINTS = 200  # 增加数据点数量以观察更长时间
        ...
        self.temperatures.append(get_temperature())
        self.setpoint.append(get_setpoint())
        ...
        self.mode_label = QLabel(f"模式: {'仿真' if DEBUG_MODE else '实际仪器'}")
        self.mode_label.setStyleSheet("color: green; font-weight: bold;" if not DEBUG_MODE else "color: blue; font-weight: bold;")
        ...
        self.log_message(f"工作模式: {'仿真' if DEBUG_MODE else '实际仪器'}")
    ...
    def reset_simulation(self):
        """重置仿真系统"""
        global sim_system
        sim_system = CryoSystem.CryoSystem('config.csv')
        self.clear_data()
        self.update_system_info()
        self.update_button.setEnabled(False)
    ...
    def update_system_info(self):
        """更新系统信息显示"""
        if DEBUG_MODE:
            self.schedule_label.setText(f"设定点计划: {sim_system.setpoint_schedule}")
            self.stabletimes_label.setText(f"稳定时间要求: {sim_system.setpoint_stabletimes}秒")
            self.current_index_label.setText(f"当前设定点索引: {sim_system.schedule_index}")
            self.current_stabletime_label.setText(f"当前稳定时间要求: {sim_system.setpoint_stabletimes[sim_system.schedule_index]}秒")
